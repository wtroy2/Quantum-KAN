...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, degrees=None, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(), symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[0, 1], sp_trainable=True, sb_trainable=True,
                 device='cpu', seed=0):
        '''
        ...
        for l in range(self.depth):
            ...
            sp_batch = KANLayer(in_dim=width[l], out_dim=width[l + 1], k=k_value, noise_scale=noise_scale, scale_base=scale_base, scale_sp=1., base_fun=base_fun, grid_eps=grid_eps, grid_range=grid_range, sp_trainable=sp_trainable,
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        for _ in pbar:
            ...
            if opt == "Adam" or opt == 'SGD' or opt == 'AdaGrad':
                pred = self.forward(dataset['train_input'][train_id].to(device))
                if sglr_avoid == True:
                    id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) == False)[0]
                    train_loss = loss_fn(pred[id_], dataset['train_label'][train_id][id_].to(device))
