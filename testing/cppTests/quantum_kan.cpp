...
inline const ArrayXd evaluate_symengine_expr_optimized(
    ...
    for (std::string &term : terms) {
        ...
        if (it != precomputed_values.end()) {
            ...
            if (base_it != precomputed_values.end()) {
                const ArrayXd& base_value = base_it->second;
                if (add_term) {
...
void evaluate_unique_xyz_expressions_optimized(
    ...
    const Eigen::ArrayXd &y_eigen,
    const Eigen::ArrayXd &z_eigen,
    std::unordered_map<std::string, ArrayXd> &evaluated_xyz_expressions, int max_exp
) {
    std::unordered_map<std::string, ArrayXd> precomputed_values;
    ...
    // for (const auto &pair : xyz_to_pvars) {
    //     RCP<const Basic> key = pair.first;
    //     const vector<RCP<const Basic>> &value = pair.second;

    //     std::cout << "Key: " << key->__str__() << std::endl;
    //     std::cout << "Values: ";
    ...
    for (const auto &pair : xyz_to_pvars) {
        std::string xyz_expr = pair.first->__str__();
        evaluated_xyz_expressions[xyz_expr] = std::move(evaluate_symengine_expr_optimized(xyz_expr, precomputed_values, temp1));
...
int compute_mse_with_penalty_categorical(int d1, int d2, int m1, int m2, double penalty_multiplier, double bias_coefficient, const std::vector<double>& x_data_train, const std::vector<double>& y_data_train, const std::vector<double>& z_data_train, const std::vector<double>& x_data_test, const std::vector<double>& y_data_test, const std::vector<double>& z_data_test, double test_multiplier,  const std::string& load_filename = "", const std::string& save_filename = "") {
    ...
    std::unordered_map<std::string, ArrayXd> evaluated_xyz_expressions_train;
    ...
    std::unordered_map<std::string, ArrayXd>  evaluated_xyz_expressions_test;
...
int main() {
    ...
    int m1 = 3;
    int m2 = 1;
    double penalty_multiplier = 15.0;
    double bias_coefficient = 0.0;
    double test_multiplier = 0.9;

    // Generate example data (dummy data, replace with actual data generation)
    int n_training_samples = 100000;
    int n_test_samples = 10000;

    ArrayXd x_data_train = ArrayXd::Random(n_training_samples);
    ArrayXd y_data_train = ArrayXd::Random(n_training_samples);
    ArrayXd z_data_train = ArrayXd::Random(n_training_samples);
    ArrayXd x_data_test = ArrayXd::Random(n_test_samples);
    ArrayXd y_data_test = ArrayXd::Random(n_test_samples);
    ArrayXd z_data_test = ArrayXd::Random(n_test_samples);

    // Normalize the data to be between 0 and 1
    auto normalize = [](ArrayXd &data) {
        double min_val = data.minCoeff();
        double max_val = data.maxCoeff();
        data = (data - min_val) / (max_val - min_val);
    };

    normalize(x_data_train);
    normalize(y_data_train);
    normalize(x_data_test);
    normalize(y_data_test);

    // Call the C++ function and measure the execution time
    auto start_time = std::chrono::high_resolution_clock::now();

    auto result = compute_mse_with_penalty_categorical(
        degree1, degree2,
        m1, m2,
        penalty_multiplier,
        bias_coefficient,
        std::vector<double>(x_data_train.data(), x_data_train.data() + x_data_train.size()),
        std::vector<double>(y_data_train.data(), y_data_train.data() + y_data_train.size()),
        std::vector<double>(z_data_train.data(), z_data_train.data() + z_data_train.size()),
        std::vector<double>(x_data_test.data(), x_data_test.data() + x_data_test.size()),
        std::vector<double>(y_data_test.data(), y_data_test.data() + y_data_test.size()),
        std::vector<double>(z_data_test.data(), z_data_test.data() + z_data_test.size()),
        test_multiplier
    );
    ...
    std::cout << "Execution time2: " << elapsed.count() << " seconds" << std::endl;
    return 0;
}